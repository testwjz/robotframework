...
class ContextAware(object):
    ...
    def is_count(self, locator, count, flag=True):
        if not self.is_visible(locator):
            return False
        elements = self.find_elements(locator)
        if flag:
            return len(elements) == int(count)
        return len(elements) >= int(count)

    def is_text_equal(self, locator, content):
        if not self.is_visible(locator):
            return False
        elements = self.find_elements(locator)
        list_content = []
        ...
        set_content = list(set(list_content))
        if len(set_content) == 1 and set_content[0] == content:
            return True
        return False

    def is_text_contain(self, locator, content):
        if not self.is_visible(locator):
            return False
        elements = self.find_elements(locator)
        try:
            for i in elements:
                if content not in i.text:
