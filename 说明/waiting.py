...
class WaitingKeywords(LibraryComponent):
    ...
    @keyword
    def wait_until_elements_count_is(self, locator, count, timeout=None,
                                     error=None):
        """获取指定位置元素列表中element数量，判断数量是否与传入的count一致
           不一致则等待
        """
        self._wait_until(
            lambda: self.is_count(locator, count),
            "count '%s' not equal after <TIMEOUT>." % count,
            timeout, error
        )

    @keyword
    def wait_until_elements_count_gte(self, locator, count, timeout=None,
                                      error=None):
        """获取指定位置元素列表中element数量，判断数量是否大于传入的count数量
           不大于则等待
        """
        self._wait_until(
            lambda: self.is_count(locator, count, False),
            "count '%s' not equal after <TIMEOUT>." % count,
            timeout, error
        )

    @keyword
    def wait_until_elements_text_equal(self, locator, content, timeout=None,
                                      error=None):
        """获取指定位置元素列表中element的text，判断所有text是否都相同且等于传入的content
           不满足条件则继续等待
           主要用于搜索后，列表结果是否与搜索匹配，用于精确搜索，且搜索结果不唯一
        """
        self._wait_until(
            lambda: self.is_text_equal(locator, content),
            "count '%s' not equal after <TIMEOUT>." % content,
            timeout, error
        )

    @keyword
    def wait_until_elements_text_contain(self, locator, content, timeout=None,
                                      error=None):
        """获取指定位置元素列表中element的text，判断所有text是否都包含content
           不满足条件则继续等待
           主要用于搜索后，列表结果是否与搜索匹配，用于模糊搜索
        """
        self._wait_until(
            lambda: self.is_text_contain(locator, content),
            "count '%s' not equal after <TIMEOUT>." % content,
            timeout, error
        )
