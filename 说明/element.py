...
class ElementKeywords(LibraryComponent):
    ...
    @keyword(name='random_click_webelements')
    def random_click_webelements(self, locator):
        """random click list of WebElements objects matching the ``locator``.
        """
        elements=self.find_elements(locator)
        random_count=random.randint(0,int(len(elements))-1)
        self.click_element(elements[random_count])

    @keyword
    def random_mouse_overs(self, locator,index=None):
        """random Simulates hovering mouse over the list elements ``locator``.
        传入index（0,1,2,3）  表示移动到列表元素
        """
        ...
        if  index == None:
            ...
            random_count=int(index)
        element = elements[random_count]
...
return !element.dispatchEvent(evt);
    ...
    def remove_location_strategy(self, strategy_name):
        ...
        # 使用 element_text_should_be 可以完成 提示信息的断言

    @keyword
    def get_msgtextdlalog(self, expected):
        '''
        消息提示框  使用 element_text_should_be 可以完成 提示信息的断言
        //*[starts -with( @class ,'layui-layer-content layui-layer-padding')]
         //*[@type='dialog']
        :param expected: 预期值
        :return:
        '''
        self.element_text_should_be("//*[@type='dialog']", expected)

    @keyword
    def get_bubbletext(self, expected):
        '''
        使用 element_text_should_be 可以完成 提示信息的断言
        气泡提示框     //*[@class='layui-layer-content']
        :param expected: 预期值
        :return:
        '''
        self.element_text_should_be("//*[@class='layui-layer-content']", expected)

    @keyword
    def get_redtext(self, expected):
        '''
        使用 element_text_should_be 可以完成 提示信息的断言
        red提示框      //*[@class='red']
        :param expected: 预期值
        :return:
        '''
        self.element_text_should_be("//*[@class='red']", expected)

    @keyword
    def Set_Tabs(self, locator):
        '''
         标签设置
         #使用click_button 可以完成标签设置
        :param locator:
        :return:
        '''
    ...
    @keyword
    def Set_TimeToTime(self, time_loc, time_value):
        '''
        设置日期
        time_loc-------格式：[time_start_loc,time_end_loc]
        public_loc-----页面上一处静态元素
        time_value-----格式：[2018-06-19,2018-12-12]
        '''
        if len(time_value) == 2:
            self.info("Typing text '%s' into text field '%s'." % (time_value[0], time_loc[0]))
            self._input_text_into_text_field(time_loc[0], time_value[0])
            self.info("Typing text '%s' into text field '%s'." % (time_value[1], time_loc[1]))
            self._input_text_into_text_field(time_loc[1], time_value[1])
    ...
    @keyword
    # 联级选择框
    def Set_CascadeSelect(self, locator, selecttitle="信用贷款|空放"):
        '''

        :param locator:
        :param tab: 联级选择内容，分割标识，默认为"|"。
        :param selecttitle: 联级选择内容。
        :return:
        '''
        self.click_button(locator);  # 点击选择框
        ...
        if len(TempData) > 0:
            for i in TempData:
                ...
                self.info(temp_loc)
                time.sleep(0.5)
                self.click_link(temp_loc);  # 点击选择框
        else:
            self.info("input null value!!!")
